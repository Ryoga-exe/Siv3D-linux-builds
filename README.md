...
available FFmpeg and other library versions. OpenCV and SoundTouch are pinned and bundled with the
SDK instead of being provided by the target system.
...
The archive contains OpenSiv3D, a pinned static OpenCV build, and a pinned shared SoundTouch build.
The corresponding SoundTouch source is included to make LGPL compliance straightforward. The
remaining Linux dependencies must be provided by the target distribution, and applications must
use the system libstdc++ ABI.
...
sudo apt-get install -y \
  ...
  libswresample-dev libswscale-dev libtiff-dev libturbojpeg0-dev \
...
When changing the Siv3D, OpenCV, or SoundTouch version, update its pinned tag and commit in the
Dockerfile and workflow together. The build checks that each tag resolves to the expected commit
before compiling.
